...
public class RxJavaTestCases {
	...
	public static void rxTestCase1() {
		...
		// Create a new Observable 被观察者or事件源 by using create()
		Observable<String> testObs = Observable.create(new Observable.OnSubscribe<String>() {
			public void call(Subscriber<? super String> sub) {
				String[] testStrings = new String[] { "dogs", "cats", "pigs" };
				for(int i = 0; i < testStrings.length; i++){
					sub.onNext(testStrings[i]);
				}
...
//		Observable<String> testObs = Observable.from(new String[] { "dogs", "cats", "pigs" });
...
//				System.out.println("Action1 case1 :" + s);
//			}
//			
//		};
		
		Action1<String> onNextAction = s -> System.out.println("Action1 case1 :" + s);
		
		testObs.subscribe(onNextAction);
	...
	public static void rxTestCase3() {
		 ...
		 * 
		 */
		testObs.flatMap(strings -> Observable.from(strings))
				.subscribe(s -> System.out.println("case3: simplified by lambda : " + s));
	}
	...
	// Create Observable by using from(), from will separate String array to
	// several Observables.
	...
	public static void rxTestCase7() {
		Observable.range(1, 20)
				.subscribe(s -> System.out.println("test range() to emit data from 1 to 20, value is : " + s));
	}
